...
export default function ParticleAura({ isSpeaking = false }) {
  ...
  const { positions, speeds, offsets, colors } = useMemo(() => {
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      // Distribute particles in a sphere / aura shape
      ...
      // Gold to warm white color range
      const t = Math.random();
      colors[i * 3] = 1.0;
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      ...
      // Spiral upward motion with speaking intensity
